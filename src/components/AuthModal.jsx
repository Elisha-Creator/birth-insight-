...
export default function AuthModal({ onClose, inline = false, onSuccess }) {
  ...
  async function handleEmail(e) {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (inline && onSuccess) onSuccess(data.user)
        if (onClose) onClose()
      }
    } catch (err) {
  ...
  const content = (
    <div className={inline ? '' : ''}>
      ...
      <div className="text-center mb-6">
        {!inline && <p className="section-label mb-2">✦ birth insight</p>}
